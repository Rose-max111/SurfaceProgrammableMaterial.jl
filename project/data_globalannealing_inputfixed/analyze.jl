...
ax1 = Axis(fig[1, 1], yscale = log, xlabel = "Sweep Time", ylabel = "Error rate", title = "Error rate v.s. Sweep Time in a 7*7 110 lattice(Fixed input)")
...
for i in 1:10:length(data.x)
    ...
    fit = fitting(Data((data_temp.x), log.(data_temp.y)))
    scatter!(ax2, [Int((i-1) / 10 + 1)], [fit.coefs[2]])
    tangent_y = fit.coefs[2] .* (data_temp.x) .+ fit.coefs[1]

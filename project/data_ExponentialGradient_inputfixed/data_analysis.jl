...
function read_data(expect_width::Float64)
    file_names = readdir("project/data_ExponentialGradient_inputfixed")
    file_data = []
    for file in file_names
        if !occursin("width=", file)
            continue
        end
        width = parse(Float64, match(r"width=(\d+\.\d+)", file).captures[1])
        if width == expect_width
            m_value = parse(Int, match(r"m=(\d+)", file).captures[1])
            velocity = parse(Float64, readline("project/data_ExponentialGradient_inputfixed/$file"))
...
my_widths = [0.8, 0.9, 1.0, 1.2, 1.5, 1.8, 2.0]
data_power_width = power_vs_width(my_widths)

fig = Figure(size=(1200, 600))
ax2 = Axis(fig[1, 2], xlabel = "width", ylabel = "slope", title = "slope v.s. width")
ax1 = Axis(fig[1, 1], xscale = log, yscale = log, title = "moving velocity v.s. depth(ExponentialGradient, input-fixed)", xlabel = "depth", ylabel = "velocity",
